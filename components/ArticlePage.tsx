  ...
  return (
    <main>
      ...
      <article className="article-shell">
        ...
        {article.summary ? <section className="article-answer" aria-labelledby="article-answer-title"><p className="eyebrow"><span /> Direct answer</p><h2 id="article-answer-title">{article.answerHeading ?? "What should buyers know?"}</h2><p>{article.summary}</p>{article.keyTakeaways?.length ? <ul>{article.keyTakeaways.map((item) => <li key={item}>{item}</li>)}</ul> : null}</section> : null}
