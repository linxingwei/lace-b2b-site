  ...
  return (
    <main>
      ...
      <article className="article-shell">
        ...
        {article.summary ? <section className="article-answer" aria-labelledby="article-answer-title"><p className="eyebrow"><span /> Direct answer</p><h2 id="article-answer-title">What defines embroidered lace applique in 2026?</h2><p>{article.summary}</p>{article.keyTakeaways?.length ? <ul>{article.keyTakeaways.map((item) => <li key={item}>{item}</li>)}</ul> : null}</section> : null}
        ...
        <div className="article-body"><p className="article-lead">{article.description}</p>{article.sections.map((section) => <section id={sectionId(section.heading)} key={section.heading}><h2>{section.heading}</h2>{section.image && section.alt ? <figure className="article-trend-figure"><Image className="article-trend-image" src={section.image} alt={section.alt} width={1536} height={1024} sizes="(max-width: 820px) 90vw, 760px" />{section.caption ? <figcaption>{section.caption}</figcaption> : null}</figure> : null}{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</section>)}</div>
