...
export default function QuoteForm() {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const message = [
      "Hello Velorace Lace, I would like a wholesale quote.",
      `Name: ${data.get("name")}`,
  ...
  return (
    <form className="quote-form" onSubmit={submit}>
      ...
      <div className="field-row"><label>WHATSAPP<input name="whatsapp" required placeholder="+1 555 000 0000" /></label><label>PRODUCT TYPE<select name="product" defaultValue=""><option value="" disabled>Select a category</option><option>3D Floral Patches</option><option>Lace Trim</option><option>Beaded Appliqués</option><option>Embroidery Appliqués</option><option>Custom Development</option></select></label></div>
