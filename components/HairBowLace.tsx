...
export function HairBowCTA({ design, placement }: { design?: HairBowDesign; placement: string }) {
  return <WhatsAppLink className="button button-whatsapp" productCategory={design ? `Hair Bow Lace / ${design.name}` : "Hair Bow Lace"} placement={placement} message={`Hello VELORACE LACE, I am interested in ${design?.name ?? "your Hair Bow Lace Collection"}. I can send the design/reference, colors and approximate quantity. Please help me check the sample and quotation options.`}>Get sample & quote on WhatsApp</WhatsAppLink>;
}
